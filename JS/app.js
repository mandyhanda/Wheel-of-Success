...
//Creating a missed variable, initialized to 0, 
//that you’ll use later to keep track of the number of guesses the player has missed 

let missed = 0;


//creating a getRandomPhraseAsArray function
...
// You’ll need to write the addPhraseToDisplay function 
//so that it can take any array of letters and add it to the display
...
whether the game has been won or lost. At the very end of the keyboard event listener, 
you’ll run this function to check if the number of letters with class “show” is equal
 to the number of letters with class “letters”. If they’re equal, 
 show the overlay screen with the “win” class and appropriate text. Otherwise, 
...
function checkWin() {
    ...
    if(missed === numOfHearts) {
        setGameOverScreen('flex', 'lose', 'Game Over!');

    } else if (missed < numOfHearts && numOfShown === numOfLetters) {
        ul.classList.add('rotate-scale-down');
        setTimeout(() => {
            setGameOverScreen('flex', 'win', 'Congratulations,You Win!');
...
function setGameOverScreen(display, className, textContent) {
    ...
    gameOverText.classList.add('rotate-scale-down');
